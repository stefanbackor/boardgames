...
import { IBAction, IBDifficulty } from '~/constants/ironbot'
...
export const Expend = () => {
  ...
  const shouldDisplayCardAction = (action?: WBAction | IBAction) => {
    return (
      action &&
      (!isRedCardAction(action) ||
        (isRedCardAction(action) &&
          hasDifficulty(IBDifficulty.RESOLVE_RED_ACTIONS)))
    )
  }
                ...
                {card[2]
                  ...
                  .map((action, index) => (
                    <Box key={index}>
                      ...
                      <CardAction key={index} action={action as IBAction} />
                    </Box>
                  ))}
              </Flex>
            </Grid>
